...
class MarketCore:
	...
	def _parse_fills(self, bytes_data: bytes, limit: int) -> List[t.FilledOrder]:
		...
		return [self.parse_fill_event(event) for event in events if event.event_flags.fill and event.native_quantity_paid > 0]
	...
	def _prepare_new_oo_account(self, owner: Keypair, balance_needed: int, signers: List[Keypair], transaction: Transaction) -> PublicKey:
			...
			transaction.add(
				create_account(
					CreateAccountParams(
						...
						lamports=self._get_lamport_need_for_sol_wrapping(limit_price, max_quantity, side, open_order_accounts),
						space=ACCOUNT_LEN,
	...
	def _build_cancel_order_by_client_id_tx(self, owner: Keypair, open_orders_account: PublicKey, client_id: int) -> Transaction:
